...
const  Page =  async ({params}: {params: Promise<{id: string}>}) => {
    ...
    // const {data: startup} = await sanityFetch({query: STARTUP_BY_ID_QUERY})
    ...
    <section className='section_container'>
        ...
        <div className='space-y-5 mt-10'>
            <div className='flex-between gap-5'>
                <Link href={`/users/${author?._id}`}  className='flex gap-2 items-center mb-3'>
                    <Image src={author?.image} alt='avatar' width={64} height={64} className='rounded-full drop-shadow-lg' />
                    <div>
                        <p className='text-20-medium'>{author?.name}</p>
                        <p className='text-16-medium !text-black-300'>@{author?.username}</p>
                    </div>
                </Link>
                <p className='category-tag capitalize'>{category}</p>
            </div>
